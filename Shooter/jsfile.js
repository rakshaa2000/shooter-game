...
window.onkeydown = function(event) {
    ...
    if((keyPr === 39|| keyPr ===68 )&& prevx<=innerWidth-rectWidth){ 
        rectX = rectX+20; //right arrow add 20 from current
    }
    else if((keyPr === 37||keyPr === 65) && prevx>10){
...
function Balls(x,y,rad,color,dx,dy) {
	...
	this.draw= function () {
		...
		if (this.rad>1){	
			...
			c1.fill();
			c1.fillStyle='black';
			c1.font="bold 16px Arial";
			//c1.textAlign="center";
			c1.fillText(this.rad,this.x-this.rad+8,this.y);
			c1.closePath();
		}
	}
...
function animate(){
	...
	for(var i=0; i<cirArray.length;i++){
		if(cirArray[i].y-cirArray[i].rad==0){
			c1.clearRect(cirArray[i].x-cirArray[i].rad-1,cirArray[i].y-cirArray[i].rad-5,cirArray[i].rad*2+5,cirArray[i].rad*2+5);
			cirArray.shift();		
		}
	}
	for (var i = 0; i < cirArray.length; i++) {
	...
	for (var i = 0; i < ballArray.length; i++) {
			...
			c1.clearRect(ballArray[i].x-ballArray[i].rad-1,ballArray[i].y-ballArray[i].rad-5,ballArray[i].rad*2+5,ballArray[i].rad*2+10);
			ballArray[i].update();	
	}
